...
**Live progress overlay (mandatory, not optional).** Each implementer command (`/implement-phase`, `/implement-fixes`) writes `{{plan_progress_path}}` during a phase, and **this command writes it too while the review is in flight**: `sub_state: "outer-review"` at the start, each finding appended as an `activity` entry of role `reviewer`, then either the block is cleared (when the phase is promoted to `completed`) or handed to the implementer with `sub_state: "applying-fixes"` (when findings remain). Without these writes the user staring at `{{plan_html_path}}` sees nothing happening for the whole review – progress writes are step gates, not commentary. After every write to `{{plan_path}}` *or* `{{plan_progress_path}}`, run:

```
python .deployed-agents/plan-renderer/bake.py --plan {{plan_path}}
```

See **Live progress overlay** + **Reviewer write checkpoints** in `.deployed-agents/conventions.md` for the schema and the exact step gates.
...
- `{{plan_progress_path}}`: write the overlay continuously while the review is running (see **Reviewer write checkpoints** in `.deployed-agents/conventions.md`). At the start, ensure `phases[N]` exists (create it if absent, seeding each item from the plan's `[w*]`/`[a*]`/`[f*]` IDs as `{ "state": "pending", "note": null }`; mirror any `✅`/`⚠️` markers already on the bullets into `state`, but `note` still seeds as `null` so later in-place note writes don't fail on shells that can't add properties to existing JSON objects, e.g. PowerShell's `PSCustomObject`), then set `sub_state: "outer-review"` and append `{role: "reviewer", msg: "outer review started"}`. As findings arise, append each as `{role: "reviewer", msg: "finding[<SEV>] <file:line> <summary>"}`. As verification commands run, append a one-line outcome the same way. At the end, **clear (delete) `phases[N]`** for any phase you promoted to `completed` – the plan's lifecycle marker is now the authoritative record (decide on any `proposed_decisions` first: fold accepted ones into `## Decisions`, drop the rest). For any phase ending in `needs-fixes` / partial, **leave the block in place** so the activity log preserves what you found, set `sub_state: "applying-fixes"`, and append `{role: "reviewer", msg: "outer review complete – K findings pending"}` so the implementer's next `/implement-fixes` run picks up cleanly. After every write to the overlay, also bake.
...
1. **Survey state – including untracked files and the live overlay.** Run `git status --short --untracked-files=all`, `git diff --check`, and `git ls-files --others --exclude-standard`. **Untracked files are part of the review surface** – do not approve if relevant implementation files are untracked and you didn't inspect them. Read the selected phase(s) in `{{plan_path}}`, including each phase's `### Work` and `### Acceptance Criteria`, plus surrounding `## Phase Flow`, `## Recommended Execution Order`, `## Open Questions`, and `## Residual Risks`. Read `{{plan_progress_path}}` if present – its `activity` log shows what the implementer worked through, and `proposed_decisions` is its channel for surfacing judgment calls for you to apply or reject. Read supporting docs in the repo when they overlap the reviewed phases.

1b. **Open the overlay (gate – do this before any code inspection).** For each phase you are about to review, edit `{{plan_progress_path}}`: ensure the `phases[N]` block exists (create it if absent per the **Reviewer write checkpoints** schema – real ISO-8601 `started_at`, items seeded from the plan's `[w*]`/`[a*]`/`[f*]` IDs as `{ "state": "pending", "note": null }`, with the plan's `✅`/`⚠️` markers folded into `state` where they indicate something other than pending — always seed `note` as `null` so later in-place note writes don't fail on shells that can't add properties to existing JSON objects, e.g. PowerShell's `PSCustomObject`); set `sub_state: "outer-review"`; append `{role: "reviewer", msg: "outer review started"}`; refresh `updated_at`. Bake. Only after this write begin step 2. The user must see the review begin in their browser before you go quiet to read code.
...
4. **Apply the review checklist (below).** Prioritize bugs, race conditions, data loss, incorrect behavior, schema/contract drift, missing production wiring, and missing tests. **As each finding crystallizes, append it to `{{plan_progress_path}}` immediately** as `{role: "reviewer", msg: "finding[<SEV>] <file:line> <summary>"}` and bake – don't save findings up for the end. The user reads the activity log in near-real-time to follow what the review is uncovering.
...
5. **Update the full status surface in `{{plan_path}}`** before final reporting (use the plan's own legend – see Plan Write Policy):
   ...
   - Walk `{{plan_progress_path}}` `proposed_decisions` for each reviewed phase: fold any accepted ones into `## Decisions` (rephrasing as needed), drop the rest. **Close out the overlay block per outcome:** for any phase promoted to `completed`, delete the entire `phases[N]` block (the plan now carries the authoritative state). For any phase ending `needs-fixes` / partial, leave the block in place (the activity log is the record of what you found), set `sub_state: "applying-fixes"`, and append `{role: "reviewer", msg: "outer review complete – K findings pending"}` so the implementer's next `/implement-fixes` run continues the same block.
   - After all writes (to `{{plan_path}}` and/or `{{plan_progress_path}}`), run the bake command once to refresh the rendered HTML.

6. **Run verification.** `{{build_cmd}}` always. `{{test_cmd}}` when tests exist. **Passing tests are necessary but not sufficient** – confirm the tests actually prove each acceptance criterion, not just that they execute. A test whose name implies coverage but whose body doesn't exercise the claimed behavior is a finding. Record which phases the verification covered and note any command that could not be run. **For each verification command, append a one-line outcome** to the overlay as `{role: "reviewer", msg: "ran <cmd>: <pass|fail summary>"}` and bake, so the user sees the gate hits land as they happen.
...
- **Plan compliance.** Each acceptance criterion is satisfied and verifiable now, or marked partial with a reason. Audit the plan's *substance*, never its structure: do not comment on whether the plan follows its canonical section layout when it is compliant, and never affirm that the structure is fine. Only flag structure when a section the phase actually needs is genuinely missing or malformed, and then as a `DOC` finding for `/review-and-fix` to repair.
