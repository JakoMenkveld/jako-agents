...
**Live progress overlay.** Each implementer command (`/implement-phase`, `/implement-fixes`) writes `{{plan_progress_path}}` during a phase. This skill reads that overlay (especially `phases[N].activity` and `phases[N].proposed_decisions`) for context, then clears the phase block when promoting the phase to `completed` – the plan's lifecycle marker is authoritative from that point. After every write to `{{plan_path}}` *or* `{{plan_progress_path}}`, run:
...
5. **Update the full status surface in `{{plan_path}}`** before final reporting (use the plan's own legend – see Plan Write Policy):
   ...
   - Walk `{{plan_progress_path}}` `proposed_decisions` for each reviewed phase: fold any accepted ones into `## Decisions` (rephrasing as needed), drop the rest. If you promoted a phase to `completed`, also clear the entire `phases[N]` block from progress.json – the plan now carries the authoritative state.
