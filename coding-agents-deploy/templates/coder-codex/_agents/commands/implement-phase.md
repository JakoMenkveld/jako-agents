...
- `items` keyed off the plan's `[w*]`/`[a*]`/`[f*]` bullet IDs, each seeded as `{ "state": "pending", "note": null }`. Seed `note` explicitly (even as `null`) so later writes that refresh a note via in-place property assignment don't fail on shells that can't add properties to an existing JSON object — PowerShell's `PSCustomObject` is the concrete case where dot-syntax mutation refuses to add a property that wasn't there at parse time.
