...
- **Exhaustive sweep per finding class on the FIRST pass.** When you flag a class of finding (unapproved abbreviation, narration comment, missing convention, banned construct, mixed line endings, etc.), grep the entire diff for every instance and list them all at once. Do not flag two `opts` occurrences on pass 4, then "discover" four more on pass 6 and three more on pass 8. Per-class checks to sweep up-front include at minimum: identifier abbreviations (compare every new identifier against the approved list in this file), narration comments, any constructs the conventions block explicitly bans, mixed line endings (`git diff --check`), trailing whitespace, missing trailing newline on edited files.
  ...
  - **Acceptance-criteria coverage sweep.** On the FIRST pass, walk every `### Acceptance Criteria` bullet (or equivalent) in the phase block and, for each, name the test file + specific test that exercises it. Orchestrator-level criteria (filter re-query, URL round-trip, error states, threshold re-fetch, run selection) require a top-level component / route / orchestrator test – sub-component unit tests do NOT satisfy them, because the wiring between components is exactly what the orchestrator-level criterion asserts. Missing top-level coverage for an orchestrator-level criterion is MAJOR (illusory coverage). Bundle every missing pairing in one finding, not one per cycle.
  - **Error-path coverage sweep.** Enumerate every error / unavailable / loading / empty branch the implementation renders or returns. For each, locate a test that drives the component into that state and asserts the output. A visibly handled branch with no driving test is MAJOR – it is unverified and a refactor will silently regress it. Bundle every missing branch in one finding.
  - **Plan-symbol literal-name sweep.** When the phase names exports verbatim (e.g. `getRuns`, `getRun`, `streamRun`), grep the code for those exact symbols. Rename mismatches (`buildStreamUrl` vs `streamRun`, `listRuns` vs `getRuns`) are MINOR plan-discrepancies – `[DOC]` if the new name is intentional, untagged MINOR if the export should be renamed to match the plan. Sweep every named symbol in one pass.
  - **Cross-cutting state-key sweep.** When the phase introduces URL query keys, cookie names, form-field names, localStorage keys, or header names, enumerate them and grep the repo for collisions across state slices. A single key serving two semantically different roles (e.g. `?runId=` used for both an active-row filter and a selected-run-detail view) is MAJOR – the second consumer silently clobbers the first on share / deep-link / refresh. Report every new key together with every existing reader/writer in one bundle.
