...
- **Em-dash sweep.** The writing-style rule bans the em dash (`—`, U+2014) in every artifact, including code comments and string literals; new banner/header comment blocks are the recurring source. Grep every file the phase touched for U+2014 (`grep -rl $'\xe2\x80\x94' <paths>`, or `perl -CSD -ne 'print if /\x{2014}/'`) and replace each with an en dash (`–`, U+2013) in one pass before the first review spawn. Past run: an em-dash finding hit all 8 files of a phase on cycle 1 purely because this sweep was skipped.
...
- **jsdom DOM-API gap sweep.** When the phase adds component code that calls a modern DOM API on a ref / Element (`scrollTo`, `scrollIntoView`, `Element.animate`, `IntersectionObserver`, `matchMedia`, etc.), jsdom may not provide it and every test that renders the component will throw. Before spawning the reviewer, grep new component code for these calls and either (a) feature-guard the call (`if (typeof el.scrollTo === "function")`), or (b) stub the API in `jest.setup.ts`. Because implementers do NOT run tests, this gap can otherwise survive build-clean and land as a BLOCKER on the first reviewer pass.
- **ARIA accessible-name sweep on role="status"/"alert"/"dialog".** When a node carries `role="status"` (or `role="alert"`/`role="dialog"`), its accessible name comes from `aria-label` / `aria-labelledby` ON THE SAME ELEMENT – `role=status` is not in the ARIA `nameFrom: contents` list, so a parent's `aria-label` does NOT propagate. Tests using `findByRole("status", { name: ... })` will fail. Verify every new `role=status/alert/dialog` element has its own `aria-label` (or aria-labelledby) on the same element.
